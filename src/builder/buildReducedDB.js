...
const inputFile = path.join(dirname, '..', 'assets', 'data', 'full.db');
const outputFile = path.join(dirname, '..', 'assets', 'data', 'movies.json');
...
const output = input.map(movie => ({
  ...
  imdbData: {
    ...
    production_countries: movie.imdbData.production_countries.map(
      country => ({ iso_3166_1: country.iso_3166_1 }),
    ),
